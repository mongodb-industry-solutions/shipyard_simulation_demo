...
const [selectedWorker, setSelectedWorker] = useState(null);
  ...
  const [modelDocs, setModelDocs] = useState({ worker: null, behavior: null, sensor: null });
  ...
  setResetStatus("");
    try {
      const res = await axios.post(`${API_URL}/generate_workers`, { n: numWorkers });
      await fetchBehavior();
...
const handleSimulate = async () => {
  ...
  try {
    ...
    const behaviorRes = await axios.get(`${API_URL}/behavior/by_run/${run_id}`);
    ...
    const latestFivePerZone = Object.values(grouped).flatMap(zoneRows =>
      zoneRows
        .sort((a, b) =>
          new Date(b.timestamp?.$date || b.timestamp) - new Date(a.timestamp?.$date || a.timestamp)
        )
        .slice(0, 5)
    );
  ...
  // Bar chart
  const shortcutCount = behavior.filter(b => b.decision === "took_shortcut").length;
  const procedureCount = behavior.filter(b => b.decision === "followed_procedure").length;
...
const runTimestamps = runs.map(run =>
  riskHistory.find(r => r.runId === run)?.runTimestamp
);
...
const lineChartData = runs.map((run, idx) =>
  Object.fromEntries([
    [
      "timestamp",
      runTimestamps[idx]
        ? new Date(runTimestamps[idx]).toLocaleString()
        : "",
    ],
    ...sessionWorkerIds.map(wid => [
      wid,
      riskHistory.find(r => r.runId === run && r.workerId === wid)?.risk_score ?? null,
    ]),
  ])
);
              ...
              <>
                <div>
                  ...
                  <pre style={{ background: "#f6f8fa", padding: 8, borderRadius: 4, fontSize: 13 }}>
                    {modelDocs.worker ? JSON.stringify(modelDocs.worker, null, 2) : "No data"}
                  </pre>
                </div>
                <div>
                  <b>worker_behavior</b>
                  <pre style={{ background: "#f6f8fa", padding: 8, borderRadius: 4, fontSize: 13 }}>
                    {modelDocs.behavior ? JSON.stringify(modelDocs.behavior, null, 2) : "No data"}
                  </pre>
                </div>
                <div>
                  <b>sensor_data</b>
                  <pre style={{ background: "#f6f8fa", padding: 8, borderRadius: 4, fontSize: 13 }}>
                    {modelDocs.sensor ? JSON.stringify(modelDocs.sensor, null, 2) : "No data"}
                  </pre>
                </div>
              </>
            )}
...
>
  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
    ...
    <span style={{ fontSize: 13, color: "#007d1a", minWidth: 80 }}>{generateStatus}</span>
  </div>
  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
    <Button
    ...
    <span style={{ fontSize: 13, color: "#007d1a", minWidth: 80 }}>{simulateStatus}</span>
  </div>
  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
    <Button
      onClick={handleReset}
    ...
    <span style={{ fontSize: 13, color: "#c00", minWidth: 80 }}>{resetStatus}</span>
  </div>
</div>
  </div>
  ...
  <LineChart data={lineChartData}>
    ...
    {sessionWorkerIds.map((wid, idx) => (
      <Line
        ...
        dot={{ r: 6, fill: `hsl(${(idx * 60) % 360},70%,40%)`, stroke: "#fff", strokeWidth: 2 }}
        activeDot={{ r: 8, fill: "#007d1a", stroke: "#fff", strokeWidth: 2 }}
        name={`Worker ${wid}`}
        connectNulls
      />
    ))}
    ...
    {behavior
      ...
      .map((row, idx) => (
        <Row key={row._id?.$oid || row._id || row.workerId || idx}>
          ...
          <Cell>{row.zone}</Cell>
          <Cell>{row.computed && row.computed.risk_score ? row.computed.risk_score : ""}</Cell>
          <Cell>
            {row.timestamp
              ? new Date(row.timestamp?.$date || row.timestamp).toLocaleString()
              : ""}
          </Cell>
        </Row>
      ))}
